...
(function (window, $, undefined) {
    ...
    //  验证是否输入全空格字符
    coreString.isSpace = function (str) {
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        return /^[ ]+$/i.test(str);
    };
    ...
    coreString.prototype.add = function () {
        console.log(Number(this));
        var array = Array.prototype.slice.call(arguments);
        return Number(this).add(array);
    };

    coreString.prototype.sub = function () {
        var array = Array.prototype.slice.call(arguments);
        return Number(this).sub(array);
    };

    coreString.prototype.mul = function () {
        var array = Array.prototype.slice.call(arguments);
        return Number(this).mul(array);
    };

    coreString.prototype.div = function () {
        var array = Array.prototype.slice.call(arguments);
        return Number(this).div(array);
    };
    ...
    // 把一个数字 四舍五入为指定精度的数值，并可格式成金额格式；该函数定义如下参数：
    //      places:     舍入操作保留的精度，默认为2
    //      symbol:     数值前的符号，默认为空
    //      thousand:   数值分隔符，默认为","
    //      decimal:    数值小数点符号,默认为"."
    coreNumber.prototype.formatMoney = function (places, symbol, thousand, decimal) {
    ...
    // 精准计算-加法
    // 传入参数必须为数值
    // 传入参数可以为array,或者多参数，会遍历所有参数相加
    coreNumber.prototype.add = function () {
        var result = this;
        var args = $.isArray(arguments[0]) ? arguments[0] : arguments;
        for (var k = 0; k < args.length; k++) {
            result = coreMath.add(result, args[k]);
        }
        return result;
    };
    ...
    // 精准计算-减法
    // 传入参数必须为数值
    // 传入参数可以为array,或者多参数，会遍历所有参数相减
    coreNumber.prototype.sub = function () {
        var result = this;
        var args = $.isArray(arguments[0]) ? arguments[0] : arguments;
        for (var k = 0; k < args.length; k++) {
            result = coreMath.sub(result, args[k]);
        }
        return result;
    };
    ...
    // 精准计算-乘法
    // 传入参数必须为数值
    // 传入参数可以为array,或者多参数，会遍历所有参数相乘
    coreNumber.prototype.mul = function () {
        var result = this;
        var args = $.isArray(arguments[0]) ? arguments[0] : arguments;
        for (var k = 0; k < args.length; k++) {
            result = coreMath.mul(result, args[k]);
        }
        return result;
    };
    //coreNumber.floatMul = function(arg1, arg2) {
    ...
    // 精准计算-除法
    // 传入参数必须为数值
    // 传入参数可以为array,或者多参数，会遍历所有参数相除
    coreNumber.prototype.div = function () {
        var result = this;
        var args = $.isArray(arguments[0]) ? arguments[0] : arguments;
        for (var k = 0; k < args.length; k++) {
            result = coreMath.div(result, args[k]);
        }
        return result;
    };
    //coreNumber.floatDiv = function(arg1, arg2) {
    //    var t1 = 0, t2 = 0, r1, r2;
    //    try {
    ...
    //      array1:         必须。 一个数组对象。
    //      array2:         必须。 一个数组对象。
    //      compareFunc:    选填。 一个方法。用作两个数组的比较
    //  返回值: array1的交集
    coreArray.intersect = function(array1, array2, compareFunc) {
        if (!coreUtil.isArray(array1) || !coreUtil.isArray(array2)) {
            return [];
        }
        var func = coreUtil.isFunction(compareFunc) ? compareFunc : coreUtil.equalsObject;
        return array1.filter(function (o) {
    ...
    coreArray.prototype.intersect = function(array2, compareFunc) { return coreArray.intersect(this, array2, compareFunc); };
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////// 
    //  javascript 的Math对象功能扩展。
    /////////////////////////////////////////////////////////////////////////////////////////////// 

    // 加法
    coreMath.add = function(v1, v2) {
    ...
    // 减法
    coreMath.sub = function(v1, v2) {
        return coreMath.add(v1, -v2);
    };

    // 乘法
    coreMath.mul = function(v1, v2) {
    ...
    coreObserver = (function () {
        ...
        Observer = function () {
            ...
            return {
                ...
                listen: function (key, fn, last) {
                    var Observer = this.create();
                    Observer.listen(key, fn, last);
                },
                trigger: function () {
                    var Observer = this.create();
                    Observer.trigger.apply(this, arguments);
                }
            };
        }();
    ...
    //  javascript 的URL操作方法。
    /////////////////////////////////////////////////////////////////////////////////////////////// 
    coreUrl.GetUrlQueryString = function(name) {
    ...
    coreUrl.getUrlArgObject = function() {
        var args = new Object();
    ...
    // 判断Object是否一样
    coreUtil.equalsObject = function (x, y) {
